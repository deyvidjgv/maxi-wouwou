const app = {
    ...
    this.productos
      ...
      .forEach((p) => {
        ...
        const imgHtml = iconUrl
          ? `<img src="${iconUrl}" style="width: 32px; height: 32px; display: block; margin: 0 auto 8px auto;">`
          : '';
  ...
  renderCarrito() {
    ...
    this.carrito.forEach((i) => {
      ...
      const imgHtml = iconUrl
        ? `<img src="${iconUrl}" style="width: 24px; height: 24px; vertical-align: middle; margin-right: 8px;">`
        : '';
      ...
      .map((p) => {
        ...
        `
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; border-top: 1px solid #e2e8f0; padding-top:15px;">
             ...
             <button class="btn btn-primary" onclick="app.completarPedidoWeb('${p.id}')" style="border-radius:8px; display:flex; align-items:center; gap:5px;">
               <i data-lucide="check-circle" style="width:16px; height:16px;"></i> Completar
             </button>
          </div>
        </div>
       `;
  ...
  abrirEdicionDescripcion() {
    const modal = document.getElementById('modalEditarDesc');
    if (!modal) return;

    // Cargar descripción actual de Firestore
    this.db
      .collection('negocios')
      .doc(this.negocioId)
      .get()
      .then((doc) => {
        if (doc.exists) {
          const desc = doc.data().descripcion || '';
          document.getElementById('textareaDesc').value = desc;
        }
  ...
  cerrarEdicionDescripcion() {
    const modal = document.getElementById('modalEditarDesc');
    if (modal) modal.style.display = 'none';
  },

  async guardarDescripcion() {
    const desc = document.getElementById('textareaDesc').value;

    if (!desc.trim()) {
      this.notificar('⚠️ La descripción no puede estar vacía');
      return;
    }

    try {
      this.notificar('💾 Guardando descripción...');
      ...
      this.notificar('✅ Descripción guardada correctamente');
      this.cerrarEdicionDescripcion();
    } catch (e) {
